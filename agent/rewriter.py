...
from agent.rag_retriever import get_retriever
...
def _build_rag_block(jd: JobDescription, top_k: int = 3) -> tuple[str, list[dict]]:
    """检索 top-K 参考 bullet，返回 (拼给 LLM 的 few-shot 文本块, references meta)。"""
    hard = " / ".join(jd.hard_requirements or [])
    kws = " / ".join(jd.keywords or [])
    query = f"{jd.title}  硬性要求：{hard}  关键词：{kws}"
    try:
        hits = get_retriever().retrieve(query, top_k=top_k, role_filter=None)
    except Exception:
        return "", []
    if not hits:
        return "", []
    lines = [f"- [{h['role_tag']} · {' / '.join(h.get('skill_tags', []))}]  {h['bullet']}" for h in hits]
    block = (
        "\n【参考案例（来自高质量 bullet 案例库，仅参考其**表述风格与量化粒度**，"
        "不得抄袭内容、不得编造简历里没有的经历）】\n" + "\n".join(lines) + "\n"
    )
    return block, hits
...
def rewrite(
    ...
    rag_block, references = ("", [])
    if use_rag:
        rag_block, references = _build_rag_block(jd, top_k=3)

    # 把 rag_references 字段从给 LLM 看的 schema 里剔除，防止它试图"填充"
    schema = RewriteResult.model_json_schema()
    schema.get("properties", {}).pop("rag_references", None)
    ...
    result = RewriteResult.model_validate_json(response.choices[0].message.content)
